...
The changelog for the repository should be maintained in a `CHANGELOG.md` file (or `NEWS.md` for R packages) at the root of the repository. Each pull request that introduces user-facing changes must include a concise entry with the PR number and author username tagged. Developer-only changes should never be included in the changelog. Example:
